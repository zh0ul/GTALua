function  help()
  ...
  print('\
      ...
      file_exists                    ( name ) \
      file2string                    ( source ) \
      file2table                     ( source, delim, noWhiteSpaceHandling, noQuoteHandling ) \
      forLineIn                      ( file, func ) \
      ...
      lua_do_v2                      (...) \
      math.average                   (...) \
      math.pointoncircle             (radius,angle,originX,originY) \
      pack                           (...) \
      shell                          (command) \
...
------------------------------------------------------------------------------------------------------------
function isUpper(s) if string.find(s,"[A-Z]") then return true ; else return false ; end ; end
function isLower(s) if string.find(s,"[a-z]") then return true ; else return false ; end ; end
------------------------------------------------------------------------------------------------------------
...
function math.pointoncircle(radius,angle,originX,originY)
  if not radius or not angle or not originX then print("usage: math.pointoncircle(radius,angle,originX,originY)\nusage: math.pointoncircle(radius,angle,originVector)") ; return originX,originY ; end
  local outType = "number"
  local z       = -1234567890
  local localpi = 3.1415926535898
  if    type(originX) == "Vector"
  then
      outType = "Vector"
      if      originX.x and originX.y and originX.z
      then    originX,originY,z = originX.x,originX.y,originX.z
      elseif  originX.x and originX.y
      then    originX,originY   = originX.x,originX.y
      end
  end
  local x = radius * math.cos(angle * localpi / 180) + originX;
  local y = radius * math.sin(angle * localpi / 180) + originY;
  if      outType == "number" then return x,y
  elseif  z ~= -1234567890    then return Vector(x,y,z)
...
function concat2files(f1,f2,sep,justify,trimEdgeWhiteSpace,trimAllWhiteSpace)
  ...
  local fd2 = io.open( f2, "r" ) ;  if ( fd2 == nil ) then print("File not found: "..f2); fd1:close() ; return ""; end
...
function  echo(...) -- Requires   string2file  ,  dumptable
    ...
    if    echo_outputFile_once_clear
    then  echo_outputFile_once = nil ; echo_outputFile_once_clear = nil
    end
    ...
    if not echo_outputFile_once_clear and echo_outputFile_once then echo_outputFile_once_clear = true ; end
